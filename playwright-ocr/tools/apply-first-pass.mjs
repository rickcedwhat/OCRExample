...
export function applyFirstPass(screenDir) {
  ...
  for (const el of pass.elements || []) {
    ...
    const crop = expandLeftForLabel(boxes, fieldBoxes);
    ...
    elements.push({
      ...
      height: crop.height,
      boxIds: el.boxIds,
      ocrRect: ocrRectFromBoxes(crop, fieldBoxes, el.type || 'field'),
  ...
  fs.writeFileSync(path.join(screenDir, 'config.ts'), `import { defineScreen } from '../../../src/screen-config.js';
